                  ...
                  const VertexData<float>& vertex_infos ,const BinaryBVH<float, TriangleData<float>>& bvh, RandomManager& rnd_manager)
{
    ...
    for(int depth = 0;;depth++)
    {
        ...
        if(is_hit)
        {
            ...
            {
                float theta = 0.5 * std::acos(1 - 2 * rnd_manager.GetRND());
                float phi = 2 * M_PI * rnd_manager.GetRND();

                float x = std::cos(phi) * std::sin(theta);
                float y = std::cos(theta);
                float z = std::sin(phi) * std::sin(theta);

                Vec3<float> wi = x * T + y * orienting_normal + z * B;
                comingRay = Ray(P + 0.001f * orienting_normal, wi);
...
int main()
{
    ...
    OBJloader load("../../../objects/bunny_plane.obj", "../../../objects");
    float scale = 0.5;
    ...
    //Camera
    float theta = 50.0f * M_PI/180.0f;
    float phi =   90.0f * M_PI/180.0f;
    float r = 7.0f;
